...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[hccore] (vcovhc) at (5.0,4.2) {vcov\_hc()};

  \node[hccore] (hcinferfun) at (5.0,1.4) {hcinfer()};

  \node[hccore] (bootpairs) at (5.0,-1.4) {boot\_pairs()};

  \node[hccore] (glsmult) at (5.0,-4.2) {gls\_mult()};

  \node[hccard, draw=hcobject, fill=hcobjectfill] (covcard) at (10.2,4.2) {%
  ...
  \node[hccard, draw=hcobject, fill=hcobjectfill] (infercard) at (10.2,1.4) {%
  ...
  \node[hccard, draw=hcobject, fill=hcobjectfill] (bootcard) at (10.2,-1.4) {%
    \textbf{S3 object: \texttt{hcinfer\_boot}}\\[-1pt]
    {\scriptsize\color{hcline} bootstrap replicates, standard errors, confidence intervals}\\[3pt]
    {\scriptsize\bfseries\ttfamily coef() \quad vcov() \quad confint() \quad plot()}
  };

  \node[hccard, draw=hcobject, fill=hcobjectfill] (glscard) at (10.2,-4.2) {%
    \textbf{S3 object: \texttt{gls\_mult}}\\[-1pt]
    {\scriptsize\color{hcline} mean and dispersion coefficients, model-based covariance, fitted variances, log-likelihood}\\[3pt]
    {\scriptsize\bfseries\ttfamily coef() \quad vcov() \quad tests() \quad confint() \quad summary() \quad plot() \quad logLik() \quad AIC() \quad BIC()}
  };

  % Distribution bus: the fitted model feeds all four entry points.
  \draw[hcbus] (fit.east) -- (2.6,0);
  \draw[hcbus] (2.6,-4.2) -- (2.6,4.2);
  \draw[hcedge] (2.6,4.2) -- (vcovhc.west);
  \draw[hcedge] (2.6,1.4) -- (hcinferfun.west);
  \draw[hcedge] (2.6,-1.4) -- (bootpairs.west);
  \draw[hcedge] (2.6,-4.2) -- (glsmult.west);
  % hcinfer() calls vcov_hc() internally.
  \draw[hcinternaledge] (hcinferfun.north) to[bend left=20]
    node[hclabel, left=0.5mm] {calls} (vcovhc.south);
  % Each entry point returns its S3 object.
  \draw[hcedge] (vcovhc.east) -- (covcard.west);
  \draw[hcedge] (hcinferfun.east) -- (infercard.west);
  \draw[hcedge] (bootpairs.east) -- (bootcard.west);
  \draw[hcedge] (glsmult.east) -- (glscard.west);
\end{tikzpicture}
\end{document}
